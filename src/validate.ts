...
import { ShapeTest } from '@shexjs/term/shexv'
...
export async function validateShex<ShapeType>({
  ...
  const validator = new ShExValidator(schema as InternalSchema, db, {
    coverage: { firstError: 'eval-simple-1err', exhaustive: '' },
  })
  ...
  try {
    const validated = (ids ?? potentialShapes).map((id) =>
      validator.validateNodeShapePair(new NamedNode(id) as Term, shapeId),
    ) as ShapeTest[]
...
function mapValidationResult(validated: ShapeTest) {
  const foundErrors = !validated.solution && ShExUtil.errsToSimple(validated)
  const foundShapes =
    validated.solution &&
    ({
  ...
  return new Promise((resolve, reject) => {
    new Parser({
      ...
      if (error) {
        ...
        resolve([RdfJsDb(n3Store), foundNodes.map((node) => node.value)])
      }
    })
  })
}
